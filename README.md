...
- `src/main/java/View/`: giao diện Java Swing
- `src/main/java/Controller/`: xử lý luồng điều khiển
...
- `COFFEE.sql`: script tạo database/schema objects
- `query.sql`: các câu query hỗ trợ
...
- JDK 22 hoặc phiên bản tương thích với cấu hình Maven hiện tại
- Maven
- Oracle Database
- SQL Developer hoặc công cụ chạy SQL tương đương
- Git
- NetBeans hoặc IntelliJ IDEA nếu muốn chạy bằng IDE
...
> Lưu ý: Trong `pom.xml`, project đang cấu hình `maven.compiler.release` là `22`, vì vậy nên dùng JDK 22 để tránh lỗi compile.
...
Code kết nối database hiện tại đang dùng:
...
Có thể chạy bằng Oracle SQL Developer:

1. Mở SQL Developer
2. Đăng nhập bằng user `TheBangClone`
3. Mở file `COFFEE.sql`
4. Chạy toàn bộ script
5. Kiểm tra các bảng đã được tạo thành công

Nếu có file `query.sql`, dùng để kiểm tra dữ liệu hoặc chạy các câu query hỗ trợ sau khi import database.
...
Nên sửa `ConnectionOracle.java` sang dạng đọc biến môi trường:
...
Nếu Oracle của bạn dùng service name thay vì SID, có thể đổi URL thành:
...
Project có chức năng gửi email bằng Gmail SMTP thông qua `javax.mail`.

File đang xử lý gửi mail:

```txt
src/main/java/Common/EmailUtil.java
```

Không nên hard-code Gmail và App Password trong file Java.
...
```bash
git filter-repo --path src/main/java/Common/EmailUtil.java --invert-paths
```
...
Trong `pom.xml` có dependency PayOS Java SDK. Nếu project có dùng PayOS, không nên hard-code các key trong code.
...
Nếu Maven không tự nhận main class, chạy:
...
Có thể chạy bằng NetBeans hoặc IntelliJ IDEA:
...
## 16. Xóa file build/log đã lỡ push lên GitHub

Chạy ở thư mục gốc repo:

```bash
git rm -r --cached target --ignore-unmatch
git rm --cached GetFunctions.class --ignore-unmatch
git rm --cached hs_err_pid*.log --ignore-unmatch
git rm --cached replay_pid*.log --ignore-unmatch
```

Nếu có thư mục IDE bị track:

```bash
git rm -r --cached .idea .vscode nbproject/private --ignore-unmatch
```
...
- `--cached` chỉ xóa khỏi Git index, không xóa file thật trên máy.
- `.gitignore` chỉ có tác dụng với file chưa bị Git track.
- Với file đã từng commit, phải dùng `git rm --cached`.

---

## 17. Quy trình chạy nhanh
