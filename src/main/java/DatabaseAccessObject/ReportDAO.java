...
public class ReportDAO {
    ...
    // =========================================================================
    // Trả về mảng Object chứa: [0]=Tổng Doanh Thu, [1]=Tổng Đơn
    public Object[] getRevenueStats(String filterType) {
        Object[] result = {0L, 0}; 
        String dateCondition = "";
        
        // Xử lý điều kiện ngày tháng theo Oracle SQL
        switch (filterType) {
            case "Hôm nay":
                dateCondition = "TRUNC(NgayDat) = TRUNC(SYSDATE)";
                break;
            case "Tuần này":
                dateCondition = "NgayDat >= TRUNC(SYSDATE, 'IW')"; // Đầu tuần (Thứ 2)
                break;
            case "Tháng này":
                dateCondition = "TRUNC(NgayDat, 'MM') = TRUNC(SYSDATE, 'MM')"; // Đầu tháng
                break;
            default:
                dateCondition = "TRUNC(NgayDat) = TRUNC(SYSDATE)"; // Tạm thời Custom cũng là hôm nay
        }

        String sql = "SELECT SUM(ThanhTien) AS TongTien, COUNT(MaDonHang) AS TongDon " +
                     "FROM DON_HANG WHERE TrangThai = N'Thành công' AND " + dateCondition;
    ...
    // Lấy dữ liệu vẽ Biểu đồ dựa theo bộ lọc
    public List<Object[]> getRevenueChartData(String filterType) {
        List<Object[]> list = new ArrayList<>();
        String dateCondition = "";
        String groupBy = "TO_CHAR(NgayDat, 'DD/MM')"; // Mặc định nhóm theo Ngày/Tháng
        String orderBy = "TRUNC(NgayDat)";

        switch (filterType) {
            case "Hôm nay":
                dateCondition = "TRUNC(NgayDat) = TRUNC(SYSDATE)";
                // [ĐÃ SỬA] Dùng TO_CHAR lấy giờ, rồi nối chuỗi || với ':00'
                groupBy = "TO_CHAR(NgayDat, 'HH24') || ':00'"; 
                orderBy = "TO_CHAR(NgayDat, 'HH24')"; // Chỉ order theo giờ để tránh lỗi gom nhóm
                break;
            case "Tuần này":
                dateCondition = "NgayDat >= TRUNC(SYSDATE, 'IW')"; // Từ đầu tuần (Thứ 2)
                break;
            case "Tháng này":
                dateCondition = "TRUNC(NgayDat, 'MM') = TRUNC(SYSDATE, 'MM')"; // Từ đầu tháng
                break;
            default:
                dateCondition = "NgayDat >= TRUNC(SYSDATE) - 6"; // Mặc định 7 ngày gần nhất
        }

        String sql = "SELECT " + groupBy + " AS NhanX, SUM(ThanhTien) AS DoanhThu " +
                     "FROM DON_HANG " +
                     "WHERE TrangThai = N'Thành công' AND " + dateCondition + " " +
    ...
    public Object[] getCustomRevenueStats(java.util.Date startDate, java.util.Date endDate) {
        Object[] result = {0L, 0}; 
        String sql = "SELECT SUM(ThanhTien) AS TongTien, COUNT(MaDonHang) AS TongDon " +
                     "FROM DON_HANG WHERE TrangThai = N'Thành công' " +
                     "AND TRUNC(NgayDat) >= ? AND TRUNC(NgayDat) <= ?";

        try (Connection conn = ConnectDatabase.ConnectionUtils.getMyConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
            
            // Chuyển java.util.Date sang java.sql.Date cho Oracle
    ...
    // 2. Lấy Dữ liệu biểu đồ theo khoảng thời gian tùy chỉnh
    ...
    // =========================================================================
    // Trả về danh sách mảng Object: {Tên Món, Loại Sản Phẩm, Số lượng bán, Doanh thu}
    public List<Object[]> getTopSellingProducts() {
        ...
        String sql = "SELECT sp.TenSanPham, lsp.TenLoaiSanPham, SUM(ct.SoLuong) AS SoLuongBan, SUM(ct.ThanhTien) AS DoanhThu " +
                     ...
                     "WHERE dh.TrangThai = N'Thành công' " +
                     "GROUP BY sp.TenSanPham, lsp.TenLoaiSanPham " +
                     "ORDER BY SoLuongBan DESC " +
                     "FETCH FIRST 10 ROWS ONLY"; // Cú pháp lấy Top 10 của Oracle 12c+
    ...
    // Lấy số lượng bán ra theo từng Danh mục (Dùng cho Biểu đồ tròn)
    ...
    // =========================================================================
    // TAB 3: HIỆU SUẤT NHÂN VIÊN CA LÀM VIỆC
    // =========================================================================
    ...
    // TAB 4: BÁO CÁO KHO 
    // =========================================================================
    // [MỚI THÊM] Hàm lấy 3 thẻ thống kê trên cùng
    public Object[] getInventoryOverviewStats() {
        Object[] result = {0L, 0, 0L}; // [Tổng vốn, Số món sắp hết, Tiền nhập tháng]
        
        // 1. Tổng vốn tồn kho = SUM(Số Lượng Tồn * Đơn Giá Bình Quân)
        String sql1 = "SELECT NVL(SUM(SoLuongTon * NVL(DonGiaBinhQuan, 0)), 0) FROM NGUYEN_LIEU";
        
        // 2. Số nguyên liệu sắp hết (SoLuongTon <= NguongCanhBao)
        String sql2 = "SELECT COUNT(*) FROM NGUYEN_LIEU WHERE SoLuongTon <= NguongCanhBao";
        
        // 3. Tiền đã nhập trong tháng này
    ...
    // Lấy danh sách Lô nguyên liệu gần hết hạn (<= 7 ngày)
    ...
    // Lấy Top 10 nguyên liệu tiêu hao nhiều nhất trong tháng (Tính cả Món chính + Topping)
    public List<Object[]> getMostUsedIngredients() {
        ...
        String sql = "SELECT TenNguyenLieu, DonViTinh, SUM(TongSuDung) AS TongTieuHao FROM ( " +
                     ...
                     "    WHERE dh.TrangThai = N'Thành công' AND tp.MaNguyenLieuTru IS NOT NULL " +
                     "      AND EXTRACT(MONTH FROM dh.NgayDat) = EXTRACT(MONTH FROM SYSDATE) " +
                     ") " +
